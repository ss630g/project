...
void init_sphere(void) {
    ...
    o->radius = 100.0;
    o->surface = SURF_NONE;
    vecMake(50.0,200.0,120.0,o->clip[o->nclips].center);
    //o->clip[o->nclips].inside = false;
    ++o->nclips;
    g.nobjects++;
    ...
    o = &g.object[g.nobjects];
    o->type = TYPE_SPHERE;
    vecMake(50.0, 200.0, 120.0, o->center);
    o->specular = true;
    o->clip[o->nclips].radius = 100.0;
    vecMake(0.5, 0.5, 0.5, o->spec);
    vecMake(.7,.9,.2, o->color);
    o->radius = 100.0;
    o->surface = SURF_NONE;
    o->inside = true;
...
void init_tri(void) {
    ...
    //o->clip[nclips].clear_Clips();
    for (int i = 0; i < 12; i++) {
        o = &g.object[g.nobjects];
        o->type = TYPE_TRIANGLE;
        vecCopy (v[face[i][0]],o->tri[0]);
        vecCopy (v[face[i][1]],o->tri[1]);
        vecCopy (v[face[i][2]],o->tri[2]);

        vecScale(100,o->tri[0]);
        vecScale(100,o->tri[1]);
        vecScale(100,o->tri[2]);
        vecTrans(0,100, 0,o->tri[0]);
        vecTrans(0,100, 0,o->tri[1]);
        vecTrans(0,100, 0,o->tri[2]);
        vecMake(0.5, 0.5, 0.5, o->spec);
        o->surface = SURF_NOISE;
        vecMake(.75,.42,.42, o->color);
        o->specular = false;
        Vec norm;
        ...
        vecMake(100.0, 200.0, 100.0, o->clip[o->nclips].center);
    ...
    o->clear_clips();
    o->type = TYPE_SPHERE;
    vecMake(100.0, 200.0, 100.0, o->center);
    o->specular = true;
   // o->clip[o->nclips].radius = 50.0;
    vecMake(0.5, 0.5, 0.5, o->spec);
    ...
    vecMake(100.0,0.0,0.0, o->clip[o->nclips].center);
    vecMake(1.0,0.0,0.0, o->clip[o->nclips].normal);
    vecNormalize(o->clip[o->nclips].normal);
    ++o->nclips;
    
    vecMake(0.0,200.0,0.0, o->clip[o->nclips].center);
    vecMake(0.0,1.0,0.0, o->clip[o->nclips].normal);
    vecNormalize(o->clip[o->nclips].normal);
    ++o->nclips;
    
    vecMake(0.0,0.0,100.0, o->clip[o->nclips].center);
    vecMake(0.0,0.0,1.0, o->clip[o->nclips].normal);
    vecNormalize(o->clip[o->nclips].normal);
    ++o->nclips;
...
void init_disk(void)
{
    ...
    for (int i = 0; i < 10; i++) {
        o = &g.object[g.nobjects];
        o->type = TYPE_DISK;
        vecMake(0.0, 0.0 + i*20.0, 0.0, o->center);
        vecMake(0.0, 1.0, 0.0, o->norm);
        o->radius = 200.0;
        o->specular = true;
        vecMake(0.2, 0.2, 0.2, o->spec);
        vecMake(0.5 + i*.5, 0.2 + i*.3, 0.0, o->color);
        o->surface = SURF_NONE;
        vecNormalize(o->norm);
        vecMake(100.0 / i+2, 100.0, 100.0, o->clip[o->nclips].center);
        o->clip[o->nclips].radius = 120.0;
        ++o->nclips;
        g.nobjects++;
    }
}
void init_poke(void) {
    ...
    //o->clip[o->nclips].inside = false;
    //++o->nclips;
    vecMake(0.0,180.0,200.0, o->clip[o->nclips].center);
    vecMake(0.0,-1.0,0.0, o->clip[o->nclips].normal);
    vecMake(0.0,200.0,200.0, o->clip[o->nclips].cent);
    o->clip[o->nclips].radius = 120.0;
    ++o->nclips;
    g.nobjects++;
   ...
   // o->clip[o->nclips].radius = 50.0;
    vecMake(0.5, 0.5, 0.5, o->spec);
    vecMake(.9,0.0,0.0, o->color);
    o->radius = 150.0;
    o->surface = SURF_NONE;
    o->inside = false;
    //o->clip[o->nclips].inside = false;
    //++o->nclips;
    vecMake(0.0,220.0,200.0, o->clip[o->nclips].center);
    vecMake(0.0,-1.0,0.0, o->clip[o->nclips].normal);
    vecMake(0.0,200.0,200.0, o->clip[o->nclips].cent);
    o->clip[o->nclips].radius = 80.0;
    ++o->nclips;
    g.nobjects++;
...
void init(void)
{
    ...
    vecMake(500.0, 500.0, 1100.0, g.from);
        ...
        if (pointInTriangle(o->tri, hit->p, &u, &v)) {
            //w = 1.0 - u - v;
                ...
                for (int i = 0; i < o->nclips; i++) {
                    ...
                    Flt dpro = vecDotProduct(o->clip[i].normal, v);
                    if (dpro > 0.0) 
                    {
                        return 0;
                    }
                    ...
                    Vec c;
                    vecSub(hit->p, o->clip[i].center, c);
                    Flt len = vecLength(c);
    ...
    if (t0 > 0.0) {
        ...
        if (o->nclips > 0.0) {
            for (int i = 0; i < o->nclips; i++) {
                ...
                Vec v;
                vecSub(hit->p,o->clip[i].center, v);
                //Flt dpro = vecDotProduct(v, o->clip[i].normal);
                ...
                if (o->clip[i].radius > 0.0) {
                    ...
                    if (len < o->clip[i].radius && o->clip[i].inside == true) {
                        goto sphere_hit2;
                        //return 0;
                    }
                    if (len > o->clip[i].radius && o->clip[i].inside == false) {
                        goto sphere_hit2;
                        //return 0;
                    }
                }
...
void trace(Ray *ray, Vec rgb, Flt weight, int level)
{
    ...
    if (o->surface == SURF_NOISE) {
        extern float noise3(float vec[3]);
                ...
                float x = (closehit.p[0]) /60;
                float y = (closehit.p[1])/60;
                float z = (closehit.p[2])/60;
                float scale = 83.0;
                ...
                ci /= 7;
                int c = gcolor * 255.0;
                    closehit.color[0] = c*.30;
                    closehit.color[1] = c*.60;
                    closehit.color[2] = c*.60;
    ...
    if(!isInShadow){
        ...
        rgb[0] += closehit.color[0] *g.diffuse[0] * dot + specHighlight;
        rgb[1] += closehit.color[1] *g.diffuse[1] * dot + specHighlight;
        rgb[2] += closehit.color[2] *g.diffuse[2] * dot + specHighlight;
    }
    return;
}
